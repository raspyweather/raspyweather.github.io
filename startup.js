...
let loader;
let boxes = [];
let docBo = createElement("div", "mainBox");
let topBar = createElement("div", "topBar");
let defaultShownBox;
...
function createEventHandler(name, element) {
    return element[name] = {
        functions: [],
        addEventListener: function (call) {
...
function processData() {
    ...
    for (let file of files) {
        ...
        if (dat == undefined || mode == undefined || sat == undefined || isNaN(sat) || isNaN(dat)) {
            console.log(file);
            console.log({dat: dat, nam: name, id: id, mode: mode});
            continue;
        }
...
function addFunctionsToImagery() {
    ...
    Imagery.GetImageLinksFromDay = function (year, month, day) {
        return this.Dates.filter(x => x.getUTCFullYear() == year && (x.getUTCMonth() + 1) == month && (x.getUTCDate() == day)).map(x => {
            return this.GetImageLinkFromId(x);
        });
    };
    ...
    Imagery.getUTCDatesFromDay = function (year, month, day) {
        return this.Dates.filter(x => x.getUTCFullYear() == year && (x.getUTCMonth() + 1) == month && (x.getUTCDate() == day));
    };
    Imagery.getUTCDatesFromMonth = function (year, month) {
        return this.Dates.filter(x => x.getUTCFullYear() == year && (x.getUTCMonth() + 1) == month);
    };

    Imagery.GetModeIndexFromString = function (modeStr) {
        return this.ImageModes.indexOf(modeStr);
    };
    Imagery.GetImagesFromMode = function (modeStr) {
        return;
    }// this.Data.filter(x => x.ModeIds.contains(this.ImageModes.indexOf(modeStr))) };
...
function createLogBox(logContent) {
    let firstBox = createElement("div", "box");
...
function createSearchGUI() {
    ...
    let queryForm = createElement("div", "inputWrapper");
    createTopBarIndex("Search", box);
    let dropDownListOperator = createDropDownList(["LIST", "SHOW"], true);
    let dropDownListModes = createDropDownList(Imagery.ImageModes.filter(x => x), false);
    let lb = createElement("div", "inputDescription");
    ...
    queryForm.appendChild(lb);
    let startDateInput = createDatePicker(new Date());
    startDateInput.onblur = function () {
        let stDate = new Date(this.value);
        if (isNaN(stDate)) {
    ...
    queryForm.appendChild(lb);
    let endDateInput = createDatePicker(new Date());
...
function createDropDownList(inputStuff, radioButton) {
    ...
    let elemWrapper = createElement("div", "elementWrapper");
    elemWrapper.Values = [];
    let shownInput = createElement("input", "shownInputBox");
    let checkBoxWrapper = createElement("div", "checkboxWrapper");
    ...
    let closeButton = createElement("input", "button");
...
function createDatePicker(initialDate) {
    ...
    let datePickerWrapperBox = createElement("div", "datePickerWrapperBox noselect");

    let datePickerInput = createElement("input", "datePickerInput");
    let datePickerBox = createElement("div", "datePickerBox noselect");
    let datePickerYearRow = createElement("div", "datePickerYearRow centerFlexBox noselect");
    let datePickerYearDec = createElement("div", "datePickerYearButton noselect");
    let datePickerYearInc = createElement("div", "datePickerYearButton noselect ");
    let datePickerYearDecDisplay = createElement("div", "datePickerYearButtonDisplay centerFlexBox noselect");
    let datePickerYearIncDisplay = createElement("div", "datePickerYearButtonDisplay centerFlexBox noselect");
    let datePickerYearDisplay = createElement("div", "datePickerYearDisplay centerFlexBox noselect");

    let datePickerMonthBox = createElement("div", "datePickerMonthBox noselect");
    let datePickerMonthDisplay = createElement("div", "datePickerMonthDisplay centerFlexBox noselect");
    let datePickerMonthRow = createElement("div", "datePickerMonthRow noselect");
    let datePickerMonthDec = createElement("div", "datePickerMonthButton noselect");
    let datePickerMonthInc = createElement("div", "datePickerMonthButton noselect");

    let datePickerMonthDecDisplay = createElement("div", "datePickerMonthButtonDisplay centerFlexBox noselect");
    let datePickerMonthIncDisplay = createElement("div", "datePickerMonthButtonDisplay centerFlexBox noselect");
    ...
    datePickerInput.update = function () {
        ...
        let dayBoxes = Array.prototype.slice.call(datePickerMonthBox.getElementsByClassName("datePickerDayBox"), []);// Array.slice(datePickerMonthBox.getElementsByClassName("datePickerDayBox"), []);
        let weeks = Imagery.DateUtility.GetWeeksOfMonth(datePickerInput.Date.getUTCFullYear(), datePickerInput.Date.getUTCMonth());
        for (let i = 0; i < 6; i++) {
            for (let ii = 0; ii < 7; ii++) {
                let datePickerInputDayBox = dayBoxes.find(x => {
                    return x.columnIdx == i && x.rowIdx == ii;
                });
...
function createThermalTemperatureView() {
    let box = createElement("div", "box");
    let queryForm = createElement("div", "inputWrapper fixedInputWrapper");
    createTopBarIndex("Thermal Analysis", box);
    let label = document.createElement("label");
    let outputTemp = createElement("div", "inputDescription");
    let inputDateLabel = createElement("div", "inputDescription");
    ...
    sel.onchange = function (x) {
        ...
        loadImageToCanvasPromised(
            Imagery.Data[value].IDs[Imagery.ImageModes.indexOf("therm")],
            box.DrawObjects.canv,
            box.DrawObjects.cont,
            box.DrawObjects.img,
            Imagery.Data[(value)].APIKey
        ).then((stuff) => {
            console.log("loaded");
            box.appendChild(stuff.Canvas);
            stuff.Canvas.onmousemove = function (mouseEvent) {
                if (!checkVisible(stuff.Canvas)) {
                    return;
                }
                let offset = stuff.Canvas.getBoundingClientRect();
                let point = {x: mouseEvent.clientX - offset.left, y: mouseEvent.clientY - offset.top};
                let rgbv = stuff.Context.getImageData(point.x, point.y, 1, 1);
...
function saveImageryState() {
    let typedArray = JSON.stringify(Imagery);
    let blob = new Blob([typedArray], {type: 'application/json'});//application/octet-binary' }); // pass a useful mime type here
...
function createStartBox() {
    ...
    let firstBox = createElement("div", "box");
    let mainCBox = createElement("div", "contentBox");
    let scrollBox = createElement("div", "scrollbox");
    ...
    sideBar.updatesideBar = function (date) {
        ...
        let createDateBar = (values, name, parent, wrapperClass, entryClass, clickFunction) => {
            if (parent[name] === undefined) {
                ...
                for (let x of values) {
                    let entry = createElement("div", entryClass);
                    entry.value = x.value;
                    entry.date = x.date;
        ...
        let clickFunction = function () {
            if (this.exists) {
                sideBar.scrollToDate(this.date);
            }
        }
    ...
    sideBar.scrollToDate = function (date) {
        ...
        let data = Imagery.GetDataFromDay(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate());
    ...
    let loadImages = function (dataArray) {
        ...
        for (let data of dataArray) {
            ...
            let satString = "&#x1f6f0;noaa" + data.sat +
                existingModes.map(x => {
                    let imageLink = Imagery.GetImageLinkFromId(data.IDs[x]);
                });
        }
    }
...
function createLoaderUI() {
    ...
    let el = createElement("div", "loaderTitle heading");
...
function createTopBarIndex(title, box) {
    let vr = createElement("div", "topBarEntry");
    vr.innerHTML = title;
